...
def crear_historial(db: Session, nombre_pdf: str, nombre_excel: str = None, pdf_data: bytes = None):
...
def obtener_historial(db):
    archivos = db.query(HistorialArchivo).order_by(HistorialArchivo.fecha_procesado.desc()).all()
    return [
        {
            "id": a.id,
